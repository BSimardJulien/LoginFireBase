...
import DropdownRHADMIN from "./dropdownRHADMIN";
...
const Navbar: React.FC<NavBarProps> = ({ isRH, isAdmin }) => {
  const auth = useRequireAuth();
  const [isOpen, setIsOpen] = useState<boolean>(false);
  const [isDropdown, setDropDown] = useState<boolean>(false);
  const [isDropdownEmployes, setDropDownEmployes] = useState<boolean>(false);
  const [isDropdownRHADMIN, setDropDownRHADMIN] = useState<boolean>(false);
  var styles = {
    bmBurgerButton: {
      position: "fixed",
      width: "8vw",
    ...
    <div className="relative inline-block text-left">
      <div>
        <button
          ...
          aria-expanded="true"
          onClick={() => {
            setDropDownRHADMIN(!isDropdownRHADMIN);
          }}
        >
          {titleRHAdmin}
      ...
      {isDropdownRHADMIN ? (
        <DropdownRHADMIN isRH={isRH} isAdmin={isAdmin} />
      ) : null}
    </div>
  );
      ...
      <div className="max-w-7xl mx-auto px-2 sm:px-6 lg:px-8">
        <div className="relative flex items-center justify-between h-16">
          ...
          <div className="flex-1 flex items-center justify-center sm:items-stretch sm:justify-start ">
            <div className="flex-shrink-0 flex items-center font-medium text-blue-500">
              {/* <img
                className="block  h-8 w-auto"
                src="https://tailwindui.com/img/logos/workflow-mark-indigo-500.svg"
                alt="Workflow"
              /> */}
              <h2>Groupe Sécurité CLB</h2>
            </div>
                ...
                <div className="relative inline-block text-left">
                  <div>
                    <button
                      ...
                      aria-expanded="true"
                      onClick={() => {
                        setDropDownEmployes(!isDropdownEmployes);
                      }}
                    >
                      Employés
                      {/* <!-- Heroicon name: solid/chevron-down --> */}
                  ...
                  {isDropdownEmployes ? <DropdownEmployes /> : null}
                </div>
                {isRH ? rhAdmin : null}
              </div>
            </div>
